...
def process_one_agg(i):
    ...
    for host in tqdm(hosts):
        ...
        np.save(os.path.join(CONFIG.PATH_PROCESSED_AGG, f"{host}_{label}_feats_{row_block}x{col_block}_{i}.npy"), host_data)
        np.save(os.path.join(CONFIG.PATH_PROCESSED_AGG, f"{host}_{label}_labels_{row_block}x{col_block}_{i}.npy"), host_times)
